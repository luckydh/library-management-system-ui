<template>
	<v-flex>
		<v-toolbar flat color="white">
     		 <v-toolbar-title>Publisher List</v-toolbar-title>
			<v-divider class="mx-2" inset vertical ></v-divider>
			<v-spacer></v-spacer>
			<v-dialog v-model="form.dialog" max-width="500px">
				<v-btn slot="activator" color="primary" dark class="mb-2">New Publisher</v-btn>
				<v-card>
					<v-card-title>
						...
						<v-container grid-list-xs>
							<v-form @keydown="form.errors.clear($event.target.name)">
								<v-layout wrap>
									...
									<v-flex xs12 sm6>
									<v-text-field v-model="form.phone" label="Phone" :error-messages="form.errors.get('phone')"></v-text-field>
									</v-flex>
									<v-flex xs12 sm6>
