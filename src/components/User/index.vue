						...
						<v-container grid-list-xs>
							<v-form @keydown="form.errors.clear($event.target.name)">
								<v-layout wrap>
									...
									<v-flex xs12 sm6>
									<v-text-field v-model="form.email" label="Email" :error-messages="form.errors.get('email')"></v-text-field>
									</v-flex>
									<v-flex xs12 sm6>
									<v-text-field v-model="form.password" label="Password" :error-messages="form.errors.get('password')"></v-text-field>
									</v-flex>
									<v-flex xs12 sm6>
									<v-text-field v-model="form.status" label="Status" :error-messages="form.errors.get('status')"></v-text-field>
									</v-flex>
								</v-layout>
							</v-form>
						</v-container>
